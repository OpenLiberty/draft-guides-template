...
The following are the prerequisites needed to go through the migration guide. 
...
For the Paketo Buildpacks command the process is different. After installing `Docker` and the `pack CLI` in the Getting Started section, you are ready to start building with a simple `war` file. For the purpose of this tutorial, the Open Liberty starter application will be used. 

[source, console]
git clone https://github.com/openliberty/guide-getting-started.git
cd guide-getting-started/finish

When setting the default builder, the following `pack config` command will be used: 
...
After this command, you can execute the `pack build` command:

`pack build your_app_name_here --env BP_JAVA_APP_SERVER=liberty`

It will build the sample app with all of the indicated features and environments. Confirm that the features and different environment variables are present in the build. A sample output of the `pack build` command will be the following: 
...
The correct build output from the `pack build` command will give you the following message at the end of: `Successfully built image myapp`. Also by checking the `Build Configuration` section of the output you can confirm if the features and environment variables defined in the `pack build` command was installed.
...
If successful the following message will display in the output: `Successfully built image your_app_name_here`. 
