...
In the `finish` directory, click the top-right `Edit Configurations`. In the pop up window, click the `+` button on the top-left. Select the `maven` in `add new configuration`. Select `finish` directory as the working directory and use
[role='command'] 
```
mvn liberty:run
```

as the command line. Click "OK" to save changes.
...
To try out the application, first go to the `finish` directory and click `run` button in the tool bar and select `liberty:run`  and deploy it to Open Liberty.
...
in the shell session where you ran the server. Alternatively, you can click `run` button in the tool bar and select `liberty:stop`  and deploy it to Open Liberty.
...
In the `start` directory, click the top-right `Edit Configurations`. In the pop up window, click the `+` button on the top-left. Select the `maven` in `add new configuration`. Select `start` directory as the working directory and use
[role='command'] 
```
mvn liberty:dev
```
...
You can click `run` button in the tool bar and select `liberty:dev` and start Open Liberty in development mode, which starts the Open Liberty server and listens for file changes.
