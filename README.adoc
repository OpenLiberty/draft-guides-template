...
// INSTUCTION: Please provide relevant tags, try to avoid inventing new ones. Use some of "MicroProfile, Maven, Docker, Kubernetes, Gradle, Java EE, Security".
:page-tags: ['Java EE']
...
// EXAMPLE:
You'll learn how to create a todo list microservice using JAX-RS, CDI and Open Liberty. You will use Maven throughout the guide to build the microservice and deploy it to run on the Open Liberty application server.
...
// If you are not using this command include statement, you'll need to add it as a subheading, ie, "== Getting started".
...
// If you are not using this command include statement, you'll need to add it as a subheading, ie, "=== Try what you'll build".
...
* Implemented basic CRUD ([hotspot=32-36 file=1]`Create`, [hotspot=39-44 file=1]`Read`, [hotspot=47-57 file=1]`Update`, and [hotspot=60-68 file=1]`Delete`) operations.
...
* Use [hotspot=27-43]`GET`, [hotspot=45-54]`POST`, [hotspot=56-71]`PUT`, and [hotspot=73-83]`DELETE` verbs to handle reading, creating, updating, and deleting resources
...
// If you are not using this command include statement, you'll need to add it as a subheading, ie, "== Building and running the application".
