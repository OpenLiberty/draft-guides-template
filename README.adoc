...
** Write a guide that follows the `README.adoc` template (APPENDIX A) in the {template-url}[`guides-template`] repository. Introduce and explain the parts of code that you removed during the last step and tell the users where to copy and paste the code. Walk through how to build the `finish` directory, run the tests, and perform any other steps that users need to run your project on their own. Make sure to follow https://github.com/OpenLiberty/draft-guides-template/wiki/Guidelines-for-Structure-and-Styles[Guidesline for Structure and Styles].
...
[source, Java, linenums, role='code_column'] 
---- 
include::finish/src/main/java/io/openliberty/guides/todolistSample/managers/TodoManagers.java[tags=**;!copyright;]
---- 
