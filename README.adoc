...
If the above `pack build` command is successful, it will have the following output. Be on the lookout for `boostrap.sh server run DefaultServer (direct)` and `Successfully built image` in the output.
...
Create the following bootstrap script (`bootstrap.sh`): 
