...
In Paketo Buildpacks, custom features can be configured on the server as well as using a volume mount to the `/features` directory that contains the feature JARs, manifests, and feature descriptor. 

The feature manifest is a `features.toml` file containing a list of features that would be installed on the server.
...
First create the feature descriptor with the following content: 
...
`pack build --path myapp --env BP_JAVA_APP_SERVER=liberty --volume /features-file-path paketo-buildpacks/eclipse-openj9 --buildpack paketo-buildpacks/java myapp`
...
Create the following script when creating UBI image. 
...
|=========
//Run through the commands and show what the output looks like
//Setup insturctions first and then the "Building the Stack Images" and include output
